namespace InveonCourseApp.Backend.Entities.Configurations.Concrete
{
    public class IdentityUserConfiguration : IEntityTypeConfiguration<IdentityUser>, IEntityConfiguration
    {
        public void Configure(EntityTypeBuilder<IdentityUser> builder)
        {
            ...
            builder.ToTable(identityUser => identityUser.HasCheckConstraint("IdentityUser_Email_MinLength_Control", "Len(Email) >= 5"));

            builder.HasIndex(identityUser => identityUser.UserName).IsUnique();
            builder.Property(identityUser => identityUser.UserName).HasColumnType("varchar").HasMaxLength(50).IsRequired();
            ...
            builder.ToTable(identityUser => identityUser.HasCheckConstraint("IdentityUser_NormalizedEmail_MinLength_Control", "Len(NormalizedEmail) >= 5"));

            builder.HasIndex(identityUser => identityUser.NormalizedUserName).IsUnique();
            builder.Property(identityUser => identityUser.NormalizedUserName).HasColumnType("varchar").HasMaxLength(50).IsRequired();
